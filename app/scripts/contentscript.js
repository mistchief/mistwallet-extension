...
function injectScript(content) {
  try {
    ...
    console.error('MistWallet: Provider injection failed.', error);
  }
}
...
async function setupStreams() {
  ...
  pump(pageMux, pageStream, pageMux, (err) =>
    logStreamDisconnectWarning('MistWallet Inpage Multiplex', err),
  );
  pump(extensionMux, extensionStream, extensionMux, (err) => {
    logStreamDisconnectWarning('MistWallet Background Multiplex', err);
  ...
  pump(legacyPageMux, legacyPageStream, legacyPageMux, (err) =>
    logStreamDisconnectWarning('MistWallet Legacy Inpage Multiplex', err),
  );
  pump(
    legacyExtensionMux,
    extensionStream,
    ...
    legacyExtensionMux,
    (err) => {
      logStreamDisconnectWarning('MistWallet Background Legacy Multiplex', err);
      notifyInpageOfStreamFailure();
    },
  );
...
function forwardTrafficBetweenMuxes(channelName, muxA, muxB) {
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `MistWallet: Muxed traffic for channel "${channelName}" failed.`,
      error,
    ),
  );
}
...
function forwardNamedTrafficBetweenMuxes(
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `MistWallet: Muxed traffic between channels "${channelAName}" and "${channelBName}" failed.`,
      error,
    ),
  );
}
...
function logStreamDisconnectWarning(remoteLabel, error) {
  console.debug(
    `MistWallet: Content script lost connection to "${remoteLabel}".`,
    error,
  );
}
...
function redirectToPhishingWarning() {
  console.debug('MistWallet: Routing to Phishing Warning component.');
